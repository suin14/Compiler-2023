...
public class LexicalAnalyser {
    ...
    private void analyseSlash() {
        ...
        if (c == '/') {     // 单行注释‘//’
            ...
            words.add(new Word("/", line));   // 只是'/'字符，非注释
            unGetChar();
        }
    }
    ...
    private void analyseCitation() {
        ...
        while ((c = getChar()) != null) {
            if (c == '"') { // 寻找到结束的‘"’
                words.add(new Word("STRCON", "\"" + buffer + "\"",line));
                return;
            } else {
                buffer.append(c);
            }
    ...
    private void analyseDigit(char pre) {
        ...
        while ((c = getChar()) != null) {
            if (Character.isDigit(c)) {
                ...
                unGetChar();
                words.add(new Word("INTCON", builder.toString(),line));
                return;
            }
        }
    }
        ...
        while ((c = getChar()) != null) {
            if (Character.isLetter(c) || c == '_' || Character.isDigit(c)) {
                ...
                if (new KeyWordMap().isKey(builder.toString())) {   // 先比对是不是关键字
                    words.add(new Word(builder.toString(),line));
                } else {
                    words.add(new Word("IDENFR", builder.toString(),line));  // 变量名
                }
                return;
            }
        }
